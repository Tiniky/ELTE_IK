<x-guest-layout>
    ...
    <form class="flex flex-col gap-4" action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <textarea name="desc" id="desc" cols="30" rows="5" class="thor-input-field">{{ old('desc', '') }}</textarea>
        @error('desc')
        ...
        @foreach ($topics as $index => $topic)
            <div class="flex items-center gap-2">
                <input type="checkbox" name="topics[]" id="topic-{{ $index }}" value="{{ $index }}"
                    class="thor-checkbox-field" @checked(in_array($index, old('topics') ?? []))>
                <label for="topic-{{ $index }}"
                    class="text-sm text-gray-900 {{ $topic->color }} rounded-full px-2 py-1 mr-1 mb-1">{{ $topic->name }}</label>
            </div>
        @endforeach
        @error('topics')
        ...
        <textarea name="content" id="content" cols="30" rows="10" class="thor-input-field">{{ old('content', '') }}</textarea>
        @error('content')
