...
function draw() {
  ...
  context.beginPath();
  context.strokeStyle = "red";  //vonal színe
  context.fillStyle = "red";    //az érték színe
  context.moveTo(0, canvas.height - bigMacPrice[0]);    //x = 0  y = canvas magassága - ár
  for (let i = 1; i < bigMacPrice.length; i++) {
    context.lineTo(i * 10, canvas.height - bigMacPrice[i]);
    if (i >= 2 && bigMacPrice[i] - bigMacPrice[i - 1] !== bigMacPrice[i - 1] - bigMacPrice[i - 2]) {      //ha nem az első elem és a két különbség megeggyezik aka nő vagy csökken
  ...
  context.beginPath();
  context.strokeStyle = "green";  //vonal színe
  context.fillStyle = "green";    //az érték színe
  context.moveTo(0, canvas.height - spotifyPrice[0]);   //x = 0  y = canvas magassága - ár
  for (let i = 1; i < spotifyPrice.length; i++) {
    context.lineTo(i * 10, canvas.height - spotifyPrice[i]);
    if (i >= 2 && spotifyPrice[i] - spotifyPrice[i - 1] !== spotifyPrice[i - 1] - spotifyPrice[i - 2]) {    ///ha nem az első elem és a két különbség megeggyezik aka nő vagy csökken
